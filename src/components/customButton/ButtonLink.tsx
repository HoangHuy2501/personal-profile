...
type ButtonLinkProps = { url: string; title: string; icon?: React.ReactNode; type?: string; tab?: boolean };
function ButtonLink({url, title, icon, type, tab}: ButtonLinkProps) {
    ...
    useEffect(() => {
        ...
        setLocale(pathLocale ?? (typeof window !== 'undefined' ? normalizeLocale(window.localStorage.getItem('language')) : 'en'));
    }, [pathname]);
    ...
    return tab || url.startsWith('http') ? <a className={classes} href={url} target={tab ? '_blank' : undefined} rel={tab ? 'noopener noreferrer' : undefined}>{content}</a> : <Link className={classes} href={localizedPath(url, locale)}>{content}</Link>;
}
